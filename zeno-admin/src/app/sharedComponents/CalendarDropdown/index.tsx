...
const CalendarDropdown: React.FC<CalendarDropdownProps> = ({ onDateChange }) => {
  ...
  return (
    <div className="relative inline-block text-white text-sm cursor-pointer">
      <div className="flex items-center justify-center bg-gray-800 border border-teal-500/50 rounded-md py-1 px-2 mb-2 sm:px-3 md:px-4" onClick={handleClick}>
      ...
      <DatePicker
        ...
        dayClassName={(date) => (startDate && endDate && date >= startDate && date <= endDate ? 'cursor-pointer bg-teal-600 text-white rounded' : 'cursor-pointer')}
      />
    </div>
  );
};
