...
const MetricCard = ({ title, value, subtext, colorClass, icon }: MetricCardProps) => {
  ...
  return (
    <div className={`xl:p-4 lg:p-2 rounded-lg ${colorClass} shadow-lg border border-gray-200 flex flex-col relative`}>
      {icon && <div className={`absolute top-4 right-4 lg:right-2 xl:right-4 ${iconColor}`}>{icon}</div>}
      <p className="text-sm xl:text-lg lg:text-md font-bold text-gray-700">{title}</p>
